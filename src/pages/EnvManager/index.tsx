...
export function EnvManagerPage() {
  ...
  const [baseText, setBaseText] = useState("API_URL=https://api.dev\nDB_HOST=localhost\nDB_USER=user\nDB_PASS=secret");
  const [compareText, setCompareText] = useState("API_URL=https://api.prod\nDB_HOST=prod.db\nDB_USER=user\n");
  ...
  const diff = useMemo(() => diffEnv(baseEnv, compareEnv), [baseEnv, compareEnv]);

  const maskedMissing = useMemo(() => maskEnv(diff.missing, maskMode), [diff.missing, maskMode]);
  const maskedExtras = useMemo(() => maskEnv(diff.extras, maskMode), [diff.extras, maskMode]);
  const maskedCommon = useMemo(() => maskEnv(diff.common, maskMode), [diff.common, maskMode]);

  const sortedCommon = useMemo(() => sortEntries(maskedCommon, sortMode, baseEnv), [maskedCommon, sortMode, baseEnv]);
  ...
  return (
    <div className={styles.page}>
      <header className={styles.header}>
        ...
        <p className="micro">base/.env와 대상 환경을 비교하고 누락/차이를 한눈에 확인하세요.</p>
      </header>
      ...
      <section className={styles.grid}>
        <div className={styles.card}>
          <div className={styles.row}>
            ...
            <button className="ghost" onClick={() => setBaseText("")}>
              지우기
            </button>
          </div>
          <textarea
        ...
        <div className={styles.card}>
          <div className={styles.row}>
            ...
            <button className="ghost" onClick={() => setCompareText("")}>
              지우기
            </button>
          </div>
          <textarea
      ...
      <section className={styles.toolbar}>
        <div className={styles.inline}>
          ...
          <select value={maskMode} onChange={(e) => setMaskMode(e.target.value as MaskMode)}>
        ...
        <button className="ghost" onClick={saveHistory}>
          Vault 저장
        </button>
      </section>
...
function sortEntries(entries: EnvEntry[], mode: SortMode, base: { entries: EnvEntry[] }) {
  if (mode === "alpha") {
  ...
  if (mode === "group") {
    ...
    return [...entries].sort((a, b) => weight(a.key) - weight(b.key) || a.key.localeCompare(b.key));
  }
  if (mode === "example") {
    const baseIndex = base.entries.reduce<Record<string, number>>((acc, e, idx) => {
      acc[e.key] = idx;
      return acc;
    }, {});
    return [...entries].sort((a, b) => (baseIndex[a.key] ?? 9999) - (baseIndex[b.key] ?? 9999));
  }
  return entries;
}
